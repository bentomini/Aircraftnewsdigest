...
# ----------------------------------------------------------------------------
# A paraphrased quote is a QUOTE failure, not a REFERENCE failure. When the
# auditor re-fetched the page and confirmed the reference number is really there
# (ref_number_found), the reference keeps VERIFIED and only the unbacked quote is
# dropped. Confidence in a reference rests on ref/rev/date/effectivity — which is
# exactly what the auditor confirmed — not on quote fidelity.
# ----------------------------------------------------------------------------
QUOTE_MISS_AUDIT = {
    "status": "not_found",
...
class TestQuoteMissKeepsReference(unittest.TestCase):
    def test_quote_miss_with_ref_confirmed_keeps_reference_verified(self):
        r, viol = v.enforce_audit_reference(ref(audit=dict(QUOTE_MISS_AUDIT)))
        self.assertEqual(r["confidence"], "VERIFIED")
        self.assertTrue(viol, "the quote failure must still be reported")

    def test_quote_miss_drops_the_quote_but_keeps_the_reference(self):
        rec = record(
            references=[ref(primary_source_url="https://ad.easa.europa.eu/ad/2026-0123",
                            audit=dict(QUOTE_MISS_AUDIT))],
            quotes=[{"text": "a paraphrase that is not in the document", "doc_title": "EASA AD",
                     "ref_number": "2026-0123", "revision_or_date": "2026-06-24",
                     "url": "https://ad.easa.europa.eu/ad/2026-0123"}])
        out, _ = v.enforce_audit_record(rec)
        self.assertEqual(out["references"][0]["confidence"], "VERIFIED")
    ...
    def test_item_stays_verified_when_only_the_quote_missed(self):
        payload = {"records": [record(references=[ref(audit=dict(QUOTE_MISS_AUDIT))])]}
        cleaned, report = v.enforce_audit(payload)
        self.assertEqual(cleaned["records"][0]["item_confidence"], "VERIFIED")

    def test_fetch_failed_still_downgrades_even_if_ref_number_found_set(self):
        # No successful independent re-fetch means no independent confirmation,
        # whatever the auditor claims about the reference number.
        a = dict(QUOTE_MISS_AUDIT, status="fetch_failed")
        r, viol = v.enforce_audit_reference(ref(audit=a))
        self.assertEqual(r["confidence"], "UNVERIFIED")
        self.assertTrue(viol)

    def test_ref_number_not_found_still_downgrades(self):
        a = dict(QUOTE_MISS_AUDIT, ref_number_found=False)
        r, viol = v.enforce_audit_reference(ref(audit=a))
        self.assertEqual(r["confidence"], "UNVERIFIED")
        self.assertTrue(viol)

    def test_quote_miss_with_trivial_excerpt_still_downgrades(self):
        # A real excerpt is the proof the auditor actually opened the page.
        a = dict(QUOTE_MISS_AUDIT, excerpt="ok")
        r, viol = v.enforce_audit_reference(ref(audit=a))
        self.assertEqual(r["confidence"], "UNVERIFIED")
        self.assertTrue(viol)
