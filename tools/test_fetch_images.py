...
"""
    ...
    def test_embed_allowlist_survives_trailing_comment(self):
        """config/fleet.yaml annotates the key inline; the list must still load."""
        cfg = fi.parse_imagery_config(
            "imagery:\n"
            "  enabled: true\n"
            "  embed_allowlist:        # public-domain agency domains only\n"
            "    - ntsb.gov\n"
            "    - faa.gov\n")
        self.assertEqual(cfg["embed_allowlist"], ["ntsb.gov", "faa.gov"])
