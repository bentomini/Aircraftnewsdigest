...
class TestSelectExclude(unittest.TestCase):
    def test_excluded_ref_is_skipped(self):
        store = {}
        cr.record_store(store, [ad_record("2026-12-11", "2026-08-03")], "2026-07-20")
        cr.record_store(store, [ad_record("2026-13-06", "2026-08-10")], "2026-07-20")
        out = cr.select(store, "2026-07-27", 90, 5, exclude={"AD:2026-12-11"})
        self.assertEqual([e["ref_number"] for e in out], ["2026-13-06"])

    def test_no_exclude_keeps_all(self):
        store = {}
        cr.record_store(store, [ad_record("2026-12-11", "2026-08-03")], "2026-07-20")
        out = cr.select(store, "2026-07-27", 90, 5)
        self.assertEqual(len(out), 1)


class TestCollectRefIds(unittest.TestCase):
    def test_collects_all_confidences(self):
        recs = [ad_record("2026-12-11", "2026-08-03", confidence="UNVERIFIED"),
                ad_record("2026-13-06", "2026-08-10", confidence="VERIFIED")]
        self.assertEqual(cr.collect_ref_ids(recs), {"AD:2026-12-11", "AD:2026-13-06"})

    def test_empty_records(self):
        self.assertEqual(cr.collect_ref_ids([]), set())
